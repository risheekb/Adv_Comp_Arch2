...
# Monitor CUDA utilization before starting
#cuda_utilization_before = []
#cuda_cores_before = []
#for i in range(device_count):
#    handle = pynvml.nvmlDeviceGetHandleByIndex(i)
#    total_cuda_cores = get_cuda_cores(handle)
#    utilization_before = get_cuda_utilization(handle)
#
#    cuda_utilization_before.append(utilization_before)
#    cuda_cores_before.append(total_cuda_cores)
#    
#    print(f"\nGPU {i}: {pynvml.nvmlDeviceGetName(handle)}")
#    print(f"Total CUDA Cores (Estimated): {total_cuda_cores}")
#    print(f"Initial CUDA Cores Utilized: ~{int(total_cuda_cores * (utilization_before / 100))} cores")
#    print("=" * 40)
